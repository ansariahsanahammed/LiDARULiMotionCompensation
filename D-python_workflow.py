...
#Define paths for input .las file and converted Pulsalyzer trajectory file (e.g.: R"C:\Folder\file.las")
input_las_file                   = R"Enter_LAS_File_Path"
input_pulsalyzer_trajectory_file = R"Enter_Trajectory_File_Path"

#Define boresight rotation angles (degrees)
x_rot = 0
y_rot = 0
z_rot = 0

#Define chunk size (number of points to process at a time) for efficient usage of memory
chunk_size = 1000000

#Parent folder path of .las file and extract las filename without extension
file_path    = os.path.dirname(os.path.abspath(input_las_file))
las_file_name = Path(input_las_file).stem

#Output .las file path (saved to same parent folder as input file)
output_las_file = os.path.join(file_path, f"python_transformed_{x_rot}_{y_rot}_{z_rot}_{las_file_name}.las")

#Function to verify linearity of trajectory data and create interpolation functions
def interpolate_trajectory(time_traj, x_traj, y_traj, z_traj, tolerance=0.999):

    #Verify linearity using R²-value
    p = Polynomial.fit(x_traj, y_traj, 1).convert()
    c, m = p.coef #y = m*x + c
    y_pred = m * x_traj + c #Predicted values based on linear fit
    ss_res = np.sum((y_traj - y_pred) ** 2) #Sum of squared residuals
    ss_tot = np.sum((y_traj - np.mean(y_traj)) ** 2) #Total sum of squares
    r_squared = 1 - (ss_res / ss_tot) if ss_tot != 0 else 1 #R²-value

    #Print R²-value and line equation
    print(f"R² : {r_squared:.10f}")
    print(f"Line equation : y = {m:.6f}*x + {c:.6f}\n")

    #Check if trajectory is linear (R² >= tolerance)
    if r_squared < tolerance:
        #If trajectory is not linear, stop execution
    ...
    #Create interpolation functions for each axis (position wrt time)
...
#Function to apply motion compensation to .las file
def apply_compensation_to_las(input_las_file, output_las_file,
                               x_rot, y_rot, z_rot,
                               trajectory_df,
                               chunk_size):
    
    #Get .las file timestamp limits for verification
    with lp.open(input_las_file) as gps_reader:

        #Load each chunk and extract timestamp limits and save to las_gps_limits list
        las_gps_limits = []
        for chunk in gps_reader.chunk_iterator(chunk_size):
            las_gps_limits.append([chunk.gps_time.min(), chunk.gps_time.max()])

    #Convert las_gps_limits to numpy array and get overall min and max timestamps from the .las file (las_gps_limits)
    las_gps_limits = np.array(las_gps_limits)
    gps_limit_min = las_gps_limits[:, 0].min()
    gps_limit_max = las_gps_limits[:, 1].max()

    #Check if timestamp limits from the .las file are within the trajectory time limits, if not stop execution    
    if gps_limit_min < trajectory_df['time'].min() or gps_limit_max > trajectory_df['time'].max():
    ...
    else: #When .las timestamp limits are within the trajectory time limits, filter trajectory data
        ...
        #Print length of filtered trajectory and its time limits
    ...
    #Interpolate trajectory data using filtered trajectory_df (position wrt time)
    ...
    with lp.open(input_las_file) as reader:
        ...
        #Open output .las file for writing, use the copied and modified header as header for output .las file
        with lp.open(output_las_file, mode='w', header=writer_header) as writer:
            #Initialize points_processed to 0
            points_processed = 0

            #Iterate through each chunk of the input .las file
            for chunk in reader.chunk_iterator(chunk_size):
                #Transform chunk and write to output file
...
#Load trajectory file as dataframe
trajectory_df = pd.read_csv(input_pulsalyzer_trajectory_file, delimiter='\t')
initial_count = len(trajectory_df)

#Drop duplicate timestamps to ensure one position per timestamp for interpolation
...
#Apply motion compensation to .las file
